...
oneTimeSetUp() {
	...
	mkdir -p foopak_modules/foopak-mock-module.git
	output=$(./foopak add --server "$project_root/mocks" foopak-mock-module.git 2>&1); exit_code=$?
...
test_should_inform_user_folder_already_exists() {
	...
	expected_error_message="could not add module: directory 'foopak_modules/foopak-mock-module.git' already exists"
