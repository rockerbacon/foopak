...
oneTimeSetUp() {
	...
	output=$(./foopak add --commit 30fc847057f4348cb6227c181de3671c19ee2457 --server "$project_root/mocks" foopak-mock-module.git 2>&1); \
		exit_code=$?
...
test_should_add_module_in_correct_path() {
	assertTrue \
		"path 'foopak_modules/foopak-mock-module.git' not found" \
		'[ -d "foopak_modules/foopak-mock-module.git" ]'
}
...
test_should_add_module_as_git_module() {
	assertContains \
		...
		"$gitmodules_content" \
		"foopak_modules/foopak-mock-module.git"
}
