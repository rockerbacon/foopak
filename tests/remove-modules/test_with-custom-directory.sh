...
oneTimeSetUp() {
	...
	submodule_add_output=$(git submodule add \
		...
		custom_dir/foopak-mock-module 2>&1
	); submodule_add_status=$?

	echo "unrelated_change" >> .gitignore
	touch unrelated_file

	output=$(./foopak remove --dir custom_dir foopak-mock-module 2>&1); \
		exit_code=$?
...
test_should_remove_module_folder() {
	assertFalse \
		"did not remove 'custom_dir/foopak-mock-module'." \
		"[ -d custom_dir/foopak-mock-module ]"
}
...
test_should_remove_module_from_cache() {
	assertFalse \
		...
		"[ -d .git/modules/custom_dir/foopak-mock-module ]"
}
